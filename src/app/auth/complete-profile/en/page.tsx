...
async function completeProfile(formData: FormData) {
  ...
  if (!user) {
    redirect("/auth/login/en?error=Please%20log%20in%20first")
  }
  ...
  if (!child_first_name || !child_last_name || !child_birthday || !child_gender) {
    redirect("/auth/complete-profile/en?error=Please%20fill%20in%20all%20required%20fields")
  }
  ...
  if (error) {
    ...
    redirect(`/auth/complete-profile/en?error=${encodeURIComponent(error.message)}`)
  }

  redirect("/dashboard/en")
}

export default async function CompleteProfilePageEn({
  searchParams,
}: {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-purple-50 to-pink-100 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden" dir="ltr">
      ...
      <div className="max-w-2xl w-full space-y-8 relative z-10">
        ...
        <div className="bg-white/70 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/50">
          ...
          <form className="space-y-6" action={completeProfile}>
            <div>
              <h2 className="text-lg font-semibold text-indigo-800 mb-4">About Your Child 🧒</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="block w-full px-4 py-3 border-2 border-indigo-100 rounded-xl bg-white/80 placeholder-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                  />
                </div>
                <div>
                ...
                <div>
                  <label htmlFor="child_birthday" className="block text-sm font-medium text-gray-700 mb-2">
                    Child's Birthday 🎂 *
                  </label>
                  <input
                    id="child_birthday"
                    ...
                    type="date"
                    required
                    className="block w-full px-4 py-3 border-2 border-indigo-100 rounded-xl bg-white/80 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                  />
                </div>
                <div>
                  <label htmlFor="child_gender" className="block text-sm font-medium text-gray-700 mb-2">
                    Child's Gender 🧸 *
                  </label>
                  <select
                    id="child_gender"
                    name="child_gender"
                    required
                    className="block w-full px-4 py-3 border-2 border-indigo-100 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                  >
            ...
            <div>
              <h2 className="text-lg font-semibold text-indigo-800 mb-4">About the Parent 👨‍👩‍👧</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                ...
                <div>
                  <label htmlFor="parent_nationality" className="block text-sm font-medium text-gray-700 mb-2">
